...
- `src/popup/` — `index.html` + `main.tsx` (mount) + `Popup.tsx` (header: logo, "Beta" badge, version from `package.json`; a primary CTA — "Open LinkedIn Jobs" in a new tab via `chrome.tabs.create`, or a "Filters are active on this page" status line instead if the active tab (checked once via `chrome.tabs.query`) is already on the jobs search page; lists blocked companies with an Unblock action, plus a search box to filter that list by name once there are more than `COMPANY_SEARCH_THRESHOLD` (5) of them — not shown below that, since it'd be more clutter than help; footer links to the GitHub issues page) + `popup.css`. The tab check needs no `tabs` permission — `content_scripts.matches` already implies host permission for that URL, which is what lets `chrome.tabs.query()` populate `url` on the matching tab at all. Brand colors (`--aw-navy`, `--aw-blue`) are approximated from `src/assets/applyw-logo.png` by eye, not pixel-sampled — nudge them if they drift from the real logo. Hidden jobs get their own full-list "screen" inside the same popup (a `view` state toggle, not a separate `chrome.windows.create` popup — keeps the UI consistent without extra window-management wiring): a linked title, bold company + location, and a relative "Hidden X ago" time (`Intl.RelativeTimeFormat`) — each truncated to one line with an ellipsis rather than wrapping — with Undo vertically centered on the row (not stacked under the text) so its distance to neighboring rows stays even regardless of text length; sorted latest-hidden-first; a "Clear all" up top; auto-returns to the main screen once the list empties out (undoing/clearing the last one) so it can't get stranded on an empty list.
