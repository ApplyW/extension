...
- `src/content/filterToggles.ts` — injects "Hide Applied"/"Hide Viewed" pills into LinkedIn's own top filter bar (Date Posted, Experience level, Easy Apply, ...), styled to match by reusing LinkedIn's own `artdeco-pill`/`search-reusables__filter-*` markup and classes. The wrapping `<ul>`'s own class is unknown, so it's located by finding an existing filter pill (`li.search-reusables__primary-filter`) and using its parent, not a guessed container selector.
